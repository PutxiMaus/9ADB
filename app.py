...
from flask import Flask, Response, abort, jsonify, render_template, request, send_file, redirect, session, has_request_context, redirect, session, has_request_context

app = Flask(__name__)
app.secret_key = os.environ.get("FLASK_SECRET_KEY", "9adb-dev-secret-change-me")
...
PROFILES_FILE = DATA_DIR / "profiles.json"  # legado, importado en /admin si no existe profile_admin
STATE_FILE = DATA_DIR / "state.json"          # legado, importado en /admin si no existe state_admin
PROFILES_NORMAL_FILE = DATA_DIR / "profiles_normal.json"
PROFILES_ADMIN_FILE = DATA_DIR / "profiles_admin.json"
STATE_NORMAL_FILE = DATA_DIR / "state_normal.json"
STATE_ADMIN_FILE = DATA_DIR / "state_admin.json"
CLIENT_AGENTS_FILE = DATA_DIR / "client_agents.json"
CLIENT_AGENT_TOKEN = os.environ.get("CLIENT_AGENT_TOKEN", "")
ADMIN_PASSWORD = os.environ.get("ADMIN_PASSWORD", "9adb-admin")
AGENT_DOWNLOADS_DIR = DATA_DIR / "agent-downloads"
AGENT_RELEASE_URL = os.environ.get("AGENT_RELEASE_URL", "https://github.com/PutxiMaus/9ADB/releases/download/agent-latest/ADB-Agent-Windows.exe")
...
for folder in (DATA_DIR, LOG_DIR, SCREENSHOT_DIR, SCREENRECORD_DIR, APK_PULL_DIR, FILES_DIR, UPLOADS_DIR, FASTBOOT_DIR, AGENT_DOWNLOADS_DIR, TEMP_DOWNLOAD_DIR, ANDROID_HOME_DIR):
...
def request_is_admin_mode() -> bool:
    try:
        return bool(has_request_context() and session.get("admin_ok"))
    except Exception:
        return False


def current_profiles_file() -> Path:
    return PROFILES_ADMIN_FILE if request_is_admin_mode() else PROFILES_NORMAL_FILE


def current_state_file() -> Path:
    return STATE_ADMIN_FILE if request_is_admin_mode() else STATE_NORMAL_FILE


def read_profiles_scoped() -> Dict[str, Any]:
    target = current_profiles_file()
    if target.exists():
        data = read_json(target, {})
    elif request_is_admin_mode() and PROFILES_FILE.exists():
        # Migración suave: lo que ya tenías de casa pasa a /admin.
...
def write_profiles_scoped(data: Dict[str, Any]) -> None:
    write_json(current_profiles_file(), data)


def read_state_scoped() -> Dict[str, Any]:
    target = current_state_file()
    if target.exists():
        data = read_json(target, DEFAULT_STATE.copy())
    elif request_is_admin_mode() and STATE_FILE.exists():
        # Migración suave: el estado antiguo pasa a /admin para no contaminar el modo agente.
...
def write_state_scoped(data: Dict[str, Any]) -> None:
    merged = DEFAULT_STATE.copy()
    merged.update(data)
    write_json(current_state_file(), merged)


def profiles() -> Dict[str, Any]:
    return read_profiles_scoped()


def save_profiles(data: Dict[str, Any]) -> None:
    write_profiles_scoped(data)
...
def state() -> Dict[str, Any]:
    return read_state_scoped()


def save_state(data: Dict[str, Any]) -> None:
    write_state_scoped(data)
...
def clear_runtime_state_files() -> None:
    for path in (STATE_NORMAL_FILE, STATE_ADMIN_FILE, STATE_FILE):
        data = read_json(path, DEFAULT_STATE.copy()) if path.exists() else DEFAULT_STATE.copy()
        if not isinstance(data, dict):
            data = DEFAULT_STATE.copy()
        data.update({
            "active_device": "",
...
def reset_adb_runtime(reason: str = "Cambio de modo") -> None:
    try:
        visor_proxy("/api/scrcpy/stop", {}, timeout=3)
    except Exception:
        pass
    try:
        run_cmd(["adb", "disconnect"], timeout=12, log_title=f"{reason}: adb disconnect")
    except Exception as exc:
        append_log(f"{reason}: adb disconnect", ok=False, output=str(exc))
    clear_runtime_state_files()
    append_log(reason, ok=True, output="Se ha parado scrcpy y se han limpiado dispositivos activos de ambos modos.")


@app.get("/")
def index():
    was_admin = bool(session.pop("admin_ok", None))
    if was_admin:
        reset_adb_runtime("Cambio de /admin a modo agente")
...
def admin_login():
    ...
    if password != ADMIN_PASSWORD:
        return admin_login_page("Contraseña incorrecta.")
    if not session.get("admin_ok"):
        reset_adb_runtime("Cambio de modo agente a /admin")
...
def admin_logout():
    if session.pop("admin_ok", None):
        reset_adb_runtime("Salida de /admin")
    return redirect("/")
...
def api_status():
    s = state()
    raw_devs = adb_devices()
    active = s.get("active_device", "")

    if not is_admin_mode():
        agent = active_client_agent()
        allowed_tunnels = set()
        if agent:
            for device in agent.get("devices", []) or []:
                try:
                    tunnel_port = int(device.get("tunnel_port") or 0)
                except Exception:
                    tunnel_port = 0
                if tunnel_port:
                    allowed_tunnels.add(f"127.0.0.1:{tunnel_port}")
        devs = [d for d in raw_devs if d.get("serial") in allowed_tunnels]
        if active and active not in allowed_tunnels:
            active = ""
            s["active_device"] = ""
            s["active_profile_id"] = ""
            save_state(s)
    ...
    return jsonify({
        ...
        "networks": local_ipv4_networks() if is_admin_mode() else [],
        "screenrecord": active_screenrecord_info() if active else {"running": False},
        "screen": screen_state_info(),
        "admin_mode": is_admin_mode(),
        "agent_required": not is_admin_mode(),
        "active_agent": agent if not is_admin_mode() else {},
        "client_agents": public_client_agents() if not is_admin_mode() else [],
    })
...
def api_connect_manual():
    data = request.get_json(silent=True) or {}
    endpoint, err = endpoint_for_current_mode(data.get("host", ""), data.get("port", 5555))
    if err:
        return jsonify({"ok": False, "message": err, "devices": adb_devices(), "state": state(), "agents": public_client_agents()})
    ok, msg = connect_endpoint(endpoint, activate=bool(data.get("activate", True)))
    return jsonify({"ok": ok, "message": msg, "devices": adb_devices(), "state": state(), "agents": public_client_agents()})
...
# =========================
# API Fase 8 · Agentes cliente
# =========================



# =========================
# Fase 8.1 · modo agente por defecto + /admin local
# =========================
...
<title>9ADB Admin</title>
<style>
html,body{{height:100%;margin:0;background:#0b0f14;color:#edf4ff;font-family:system-ui,-apple-system,Segoe UI,sans-serif}}
body{{display:grid;place-items:center}}
form{{width:min(420px,calc(100vw - 32px));background:#111821;border:1px solid rgba(255,255,255,.09);border-radius:22px;padding:24px;box-shadow:0 20px 60px rgba(0,0,0,.35)}}
h1{{margin:0 0 8px;font-size:24px}}
p{{color:#90a0b3}}
.err{{color:#ff6678}}
input,button{{width:100%;box-sizing:border-box;border-radius:13px;padding:12px;margin-top:12px;font:inherit}}
input{{background:#070b10;color:#edf4ff;border:1px solid rgba(255,255,255,.12)}}
button{{border:0;background:#74b4ff;color:#06101d;font-weight:800;cursor:pointer}}
a{{color:#74b4ff}}
</style>
</head>
<body>
...
<h1>9ADB Admin</h1>
<p>Modo directo del servidor. Usa la red de casa del servidor, sin agente.</p>
{err}
<input name="password" type="password" placeholder="Contraseña admin" autofocus>
<button type="submit">Entrar</button>
<p><a href="/">Volver al modo agente</a></p>
</form>
</body>
</html>"""


def active_client_agent() -> Dict[str, Any]:
    agents = public_client_agents()
    s = state()
    active_id = s.get("active_client_agent_id") or ""
    online = [a for a in agents if a.get("online")]
    if active_id:
        for agent in agents:
            if agent.get("id") == active_id and agent.get("online"):
                return agent
    if online:
        agent = online[0]
        s["client_network_mode"] = "client"
        s["active_client_agent_id"] = agent.get("id", "")
        save_state(s)
        return agent
    return {}


def client_agent_network_devices(agent: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
    agent = agent or active_client_agent()
    if not agent:
        return []
    connected_serials = {d.get("serial") for d in adb_devices()}
    active = active_serial()
    items: List[Dict[str, Any]] = []
    for device in agent.get("devices", []) or []:
        ip_text = str(device.get("ip") or "").strip()
        try:
            port = int(device.get("port") or 5555)
        except Exception:
            port = 5555
        try:
            tunnel_port = int(device.get("tunnel_port") or 0)
            ...
            tunnel_port = 0
        tunnel_endpoint = f"127.0.0.1:{tunnel_port}" if tunnel_port else ""
        label = device.get("name") or device.get("serial") or f"{ip_text}:{port}"
        items.append({
            "ip": ip_text,
            "port": port,
            "mac": "",
            "vendor": "",
            "source": f"agente: {agent.get('name') or agent.get('id')}",
            "adb_port_open": bool(device.get("adb_open", True)),
            "adb_connected": bool(tunnel_endpoint and tunnel_endpoint in connected_serials),
            "active": bool(tunnel_endpoint and active == tunnel_endpoint),
            "adb_status": "túnel" if tunnel_endpoint else "sin túnel",
...
def find_agent_device(ip_text: str = "", agent_id: str = "", tunnel_port: int = 0) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    agents = public_client_agents()
    if agent_id:
        agents = [a for a in agents if a.get("id") == agent_id]
    else:
        active = active_client_agent()
        agents = [active] if active else agents
    for agent in agents:
        for device in agent.get("devices", []) or []:
            try:
                dev_tunnel = int(device.get("tunnel_port") or 0)
            except Exception:
                dev_tunnel = 0
            if tunnel_port and dev_tunnel == tunnel_port:
                return agent, device
            if ip_text and str(device.get("ip") or "") == ip_text:
                return agent, device
    return {}, {}
...
def useful_network_devices(devices: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """Evita enseñar 192.168.x.1-.254 entero. Solo deja cosas realmente útiles para ADB."""
    ...
    for item in devices or []:
        ...
        key = serial or ip_text
        if not key or key in seen:
            continue

        is_useful = bool(
            item.get("active")
            or item.get("adb_connected")
            or item.get("adb_port_open")
            or item.get("profile_id")
            or item.get("profile")
            or str(item.get("adb_status") or "").lower() in {"device", "túnel", "tunnel"}
        )
...
def endpoint_for_current_mode(host_or_ip: str, port: Any = 5555) -> Tuple[str, str]:
    endpoint = normalize_endpoint(host_or_ip, port)
    if is_admin_mode():
        return endpoint, ""
    if not endpoint:
        return "", "Falta IP:puerto."
    ip_text = endpoint.split(":", 1)[0]
    agent, device = find_agent_device(ip_text=ip_text)
    if not agent:
        return "", "Inicia 9ADB-Agent en el PC cliente para usar esa red."
    try:
        tunnel_port = int(device.get("tunnel_port") or 0)
    except Exception:
        tunnel_port = 0
    if not tunnel_port:
        return "", "El agente ha detectado el Android, pero todavía no hay túnel activo."
    return f"127.0.0.1:{tunnel_port}", ""


def set_agent_as_default(agent_id: str) -> None:
    s = state()
    s["client_network_mode"] = "client"
    s["active_client_agent_id"] = agent_id
    save_state(s)




@app.get("/agent/download/windows")
@app.get("/agent/downloads/windows")
def agent_download_windows():
    try:
        local = AGENT_DOWNLOADS_DIR / "ADB-Agent-Windows.exe"
        repo_local = BASE_DIR / "agent" / "dist" / "ADB-Agent-Windows.exe"
        old_local = AGENT_DOWNLOADS_DIR / "9ADB-Agent-Windows.exe"
        old_repo_local = BASE_DIR / "agent" / "dist" / "9ADB-Agent-Windows.exe"
        if local.exists():
            return send_file(local, as_attachment=True, download_name="ADB-Agent-Windows.exe")
        if repo_local.exists():
            return send_file(repo_local, as_attachment=True, download_name="ADB-Agent-Windows.exe")
        if old_local.exists():
            return send_file(old_local, as_attachment=True, download_name="ADB-Agent-Windows.exe")
        if old_repo_local.exists():
            return send_file(old_repo_local, as_attachment=True, download_name="ADB-Agent-Windows.exe")
        return redirect(AGENT_RELEASE_URL, code=302)
    except Exception as exc:
        append_log("Descarga agente Windows", ok=False, output=str(exc))
        return jsonify({
            "ok": False,
            "message": "No se pudo servir el agente. Ejecuta el workflow Build Windows Agent o copia el EXE a /srv/data/adb/agent-downloads/ADB-Agent-Windows.exe",
            "error": str(exc),
            "release_url": AGENT_RELEASE_URL,
        }), 500

@app.get("/agent/download/info")
def agent_download_info():
    host = request.host.split(":")[0]
    return jsonify({
        "ok": True,
        "windows_exe": "/agent/download/windows",
        "release_url": AGENT_RELEASE_URL,
        "server_url": f"http://{host}:20009",
        "agent_required": not is_admin_mode(),
        "admin_mode": is_admin_mode(),
        "active_agent": active_client_agent() if not is_admin_mode() else {},
    })


@app.post("/api/client-agent/select")
def api_client_agent_select():
    data = request.get_json(silent=True) or {}
    agent_id = clean_client_id(data.get("agent_id") or "")
    agents = {a.get("id"): a for a in public_client_agents()}
    if not agent_id or agent_id not in agents:
        return jsonify({"ok": False, "message": "Agente no encontrado.", "agents": public_client_agents()})
    set_agent_as_default(agent_id)
    return jsonify({"ok": True, "message": f"Usando la red de {agents[agent_id].get('name') or agent_id}.", "agents": public_client_agents(), "state": state()})


@app.get("/api/client-agents")
def api_client_agents_list():
    return jsonify({
        "ok": True,
        "token_required": bool(CLIENT_AGENT_TOKEN),
        "agents": public_client_agents(),
    })


@app.post("/api/client-agent/register")
def api_client_agent_register():
    data = request.get_json(silent=True) or {}
    ok_token, token_msg = verify_client_agent_payload(data)
    if not ok_token:
        append_log("Agente cliente rechazado", ok=False, output=token_msg)
        return jsonify({"ok": False, "message": token_msg}), 403

    agent_id = clean_client_id(data.get("id") or data.get("client_id") or "")
    agent = {
        "id": agent_id,
        "name": str(data.get("name") or agent_id).strip()[:80],
        "host": str(data.get("host") or "").strip()[:120],
        "user": str(data.get("user") or "").strip()[:80],
        "platform": str(data.get("platform") or "").strip()[:80],
        "local_ip": str(data.get("local_ip") or "").strip()[:64],
        "network": str(data.get("network") or "").strip()[:64],
        "ssh_target": str(data.get("ssh_target") or "").strip()[:160],
        "last_seen": now_text(),
        "last_seen_ts": time.time(),
        "devices": [normalize_agent_device(d) for d in (data.get("devices") or []) if isinstance(d, dict)],
        "message": str(data.get("message") or "").strip()[:500],
        "online": True,
    }
    store = client_agents_data()
    store["agents"][agent_id] = agent
    save_client_agents_data(store)

    current = state().get("active_client_agent_id") or ""
    online = {a.get("id"): a for a in public_client_agents() if a.get("online")}
    if not current or current not in online:
        set_agent_as_default(agent_id)

    append_log("Agente cliente", ok=True, output=f"{agent['name']} · {len(agent['devices'])} dispositivos · {agent['local_ip']}")
    return jsonify({"ok": True, "message": "Agente registrado.", "agent": dict(agent, token=None), "state": state()})


@app.post("/api/client-agent/connect")
def api_client_agent_connect():
    data = request.get_json(silent=True) or {}
    agent_id = clean_client_id(data.get("agent_id") or "")
    ...
    try:
        ...
        tunnel_port = 0
    if not agent_id or not tunnel_port:
        return jsonify({"ok": False, "message": "Falta agente o puerto de túnel.", "agents": public_client_agents()})
    ...
    if not agent.get("online"):
        return jsonify({"ok": False, "message": "El agente no está online.", "agents": public_client_agents()})

    endpoint = f"127.0.0.1:{tunnel_port}"
    ok, msg = connect_endpoint(endpoint, activate=True)
    if ok:
        label = str(data.get("label") or f"{agent.get('name', 'Cliente')} · {endpoint}")
        add_recent_device(endpoint, label=label)
    return jsonify({"ok": ok, "message": msg, "devices": adb_devices(), "state": state(), "agents": public_client_agents()})


@app.post("/api/client-agent/remove")
def api_client_agent_remove():
    data = request.get_json(silent=True) or {}
    agent_id = clean_client_id(data.get("agent_id") or "")
    store = client_agents_data()
    if agent_id in store.get("agents", {}):
        store["agents"].pop(agent_id, None)
        save_client_agents_data(store)
    return jsonify({"ok": True, "message": "Agente eliminado.", "agents": public_client_agents()})


@app.get("/api/client-agent/guide")
def api_client_agent_guide():
    host = request.host.split(":")[0]
    return jsonify({
        "ok": True,
        "server_url": f"http://{host}:20009",
        "example": f"python agent/9adb_agent.py --server http://{host}:20009 --ssh TU_USUARIO@{host} --scan --auto-tunnel",
        "token_required": bool(CLIENT_AGENT_TOKEN),
        "notes": [
            "Ejecuta el agente en el PC que está en la WiFi del Android.",
            "El agente escanea la red local del PC cliente.",
            "El agente abre un túnel SSH inverso hacia el servidor.",
            "El servidor conecta con adb a 127.0.0.1:puerto_tunel.",
        ],
    })
...
def api_network_scan():
    if not is_admin_mode():
        ...
        if not agent:
            return jsonify({
                "ok": False,
                "message": "Necesitas abrir 9ADB-Agent en este PC para usar esta web.",
                "devices": [],
                "state": state(),
                "network": "sin agente",
                "agent_required": True,
        ...
        result = client_agent_network_devices(agent)
        s = state()
        s["last_network_scan"] = result
        s["last_network_scan_at"] = now_text()
        s["last_network_range"] = agent.get("network") or "red del agente"
        save_state(s)
        return jsonify({"ok": True, "devices": result, "state": state(), "network": s["last_network_range"], "agent": agent, "agents": public_client_agents()})

    data = request.get_json(silent=True) or {}
    network = (data.get("network") or "").strip()
    port = int(data.get("port") or 5555)
    result = useful_network_devices(scan_network(network, port=port))
    s = state()
    s["last_network_scan"] = result
    s["last_network_scan_at"] = now_text()
    s["last_network_range"] = network or default_network()
    save_state(s)
    return jsonify({"ok": True, "devices": result, "state": state(), "network": s["last_network_range"], "message": f"Escaneo terminado: {len(result)} dispositivo(s) útiles."})
...
def api_network_last():
    if not is_admin_mode():
        agent = active_client_agent()
        if agent:
            devices = client_agent_network_devices(agent)
            return jsonify({
                "ok": True,
                "devices": devices,
                "at": agent.get("last_seen", ""),
                "last_network_range": agent.get("network") or "red del agente",
                "default_network": agent.get("network") or "",
                "agent": agent,
                "agent_required": True,
                "agents": public_client_agents(),
            })
    ...
    return jsonify({
        "ok": True,
        "devices": useful_network_devices(s.get("last_network_scan", [])),
...
def api_network_connect():
    ...
    endpoint, err = endpoint_for_current_mode(ip_text, port)
    if err:
        return jsonify({"ok": False, "message": err, "devices": adb_devices(), "state": state(), "agents": public_client_agents()})
    ok, msg = connect_endpoint(endpoint, activate=True)
    return jsonify({"ok": ok, "message": msg, "devices": adb_devices(), "state": state(), "agents": public_client_agents()})
...
def adb_device_get_state(serial: str, timeout: int = 10) -> Tuple[bool, str]:
    if not serial:
        return False, "No hay dispositivo activo."
    ok, out, _ = run_cmd(["adb", "-s", serial, "get-state"], timeout=timeout)
    text = (out or "").strip()
    if ok and "device" in text.lower():
        return True, "device"
    if "unauthorized" in text.lower():
        return False, "El móvil está sin autorizar. Acepta la huella RSA/ADB en la pantalla del Android."
    if "offline" in text.lower():
        return False, "El dispositivo aparece offline. Desconecta y vuelve a conectar ADB."
    if "timeout" in text.lower():
        return False, "ADB no responde con este dispositivo. Prueba a desconectar/conectar de nuevo."
    return False, text or "ADB no puede hablar con el dispositivo activo."


def require_ready_active() -> Tuple[str, Optional[str]]:
    serial, err = require_active()
    if err:
        return "", err
    ok, msg = adb_device_get_state(serial, timeout=10)
    if not ok:
        return "", msg
    return serial, None
...
# =========================
# Fase 8 · Red del cliente con agente + túnel SSH inverso
# =========================

def client_agents_data() -> Dict[str, Any]:
...
def save_client_agents_data(data: Dict[str, Any]) -> None:
    write_json(CLIENT_AGENTS_FILE, data)


def verify_client_agent_payload(data: Dict[str, Any]) -> Tuple[bool, str]:
    if not CLIENT_AGENT_TOKEN:
        return True, ""
    token = str(data.get("token") or request.headers.get("X-9ADB-Agent-Token") or "").strip()
    if token != CLIENT_AGENT_TOKEN:
        return False, "Token de agente incorrecto."
    return True, ""


def clean_client_id(value: str) -> str:
    value = re.sub(r"[^a-zA-Z0-9_.-]+", "-", (value or "").strip())[:80]
    return value or uuid.uuid4().hex[:12]


def purge_old_client_agents(data: Dict[str, Any], max_age_seconds: int = 180) -> Dict[str, Any]:
    now = time.time()
    agents = data.get("agents", {})
    clean = {}
    for agent_id, agent in agents.items():
        try:
            ts = float(agent.get("last_seen_ts") or 0)
        except Exception:
            ts = 0
        agent["online"] = bool(ts and (now - ts) <= max_age_seconds)
        clean[agent_id] = agent
    data["agents"] = clean
    return data


def public_client_agents() -> List[Dict[str, Any]]:
    data = purge_old_client_agents(client_agents_data())
    save_client_agents_data(data)
    items = []
    for agent_id, agent in data.get("agents", {}).items():
        ...
        public["id"] = agent_id
        items.append(public)
    items.sort(key=lambda x: (not x.get("online"), x.get("name", ""), x.get("id", "")))
    return items


def normalize_agent_device(device: Dict[str, Any]) -> Dict[str, Any]:
    ip_text = str(device.get("ip") or "").strip()
    port = int(device.get("port") or 5555)
    tunnel_port = device.get("tunnel_port")
    try:
        tunnel_port = int(tunnel_port) if tunnel_port else 0
    except Exception:
        tunnel_port = 0
    return {
        "ip": ip_text,
        "port": port,
        "serial": str(device.get("serial") or f"{ip_text}:{port}").strip(),
        "name": str(device.get("name") or device.get("label") or "").strip(),
        "adb_open": bool(device.get("adb_open", True)),
        "tunnel_port": tunnel_port,
        "tunnel_endpoint": f"127.0.0.1:{tunnel_port}" if tunnel_port else "",
        "status": str(device.get("status") or ("tunnel" if tunnel_port else "detected")).strip(),
        "last_seen": str(device.get("last_seen") or now_text()).strip(),
    }
...
def api_json_error(exc):
    if request.path.startswith("/api/"):
        append_log("Error interno API", ok=False, output=f"{request.path}: {exc}")
        return jsonify({"ok": False, "message": f"Error interno: {exc}"}), 500
    raise exc
